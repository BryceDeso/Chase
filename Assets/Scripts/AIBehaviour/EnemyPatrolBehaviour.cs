...
public class EnemyPatrolBehaviour : MonoBehaviour
{
    ...
    public void PatrolingState()
    {
        ...
        if (transform.position != Points[current].position)
        {
            ...
            transform.position = Vector3.MoveTowards(transform.position, Points[current].position, GetComponent<EnemyDecisionBehaviour>().MovementSpeed * Time.deltaTime);

            //looks at the point it's moving towards
            _direction = (Points[current].position - transform.position).normalized;
            _lookRotation = Quaternion.LookRotation(_direction);
            transform.rotation = Quaternion.Slerp(transform.rotation, _lookRotation, Time.deltaTime * GetComponent<EnemyDecisionBehaviour>().RotationSpeed);
        }
    }
}
